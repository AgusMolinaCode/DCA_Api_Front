...
import { CalendarIcon, TargetIcon, WalletIcon } from 'lucide-react';
  ...
  return (
    <div className="w-full">
      <Card className="bg-zinc-800 border-zinc-600">
        ...
        <CardContent>
          {error ? (
            <div className="text-red-400 p-4">Error al cargar las bolsas de inversión: {error}</div>
          ) : bolsas.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {bolsas.map((bolsa) => (
                <Card key={bolsa.id} className="bg-zinc-700/40 border-zinc-600 overflow-hidden">
                  <div className="h-1 w-full bg-blue-500"></div>
                  <CardContent className="p-4">
                    <div className="flex justify-between items-start mb-2">
                      <h3 className="font-medium text-zinc-100 truncate">{bolsa.name}</h3>
                    ...
                    {bolsa.description && (
                      <p className="text-zinc-400 text-sm mb-3 line-clamp-2">{bolsa.description}</p>
                    )}
                    ...
                    <div className="mt-3 pt-3 border-t border-zinc-600 flex justify-between items-center">
                      ...
                      <a href={`/dashboard/bolsas/${bolsa.id}`} className="text-xs text-blue-400 hover:text-blue-300 transition-colors">
                        Ver detalles →
                      </a>
                    </div>
                  </CardContent>
